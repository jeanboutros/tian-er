...
PostgreSQL `max_connections` is configured at **50** (generous headroom for 25–30 typical connections). This covers normal operation plus administrative connections (pg_basebackup, operator `psql` sessions).
...
`raw_packets` chunks older than 7 days are automatically compressed by TimescaleDB:
...
TimescaleDB continuous aggregate that pre-computes per-MAC, per-5-minute-bucket statistics from `raw_packets`:
...
Database roles are initialized once by `db/roles/init-roles.sql`, executed by `deploy/scripts/install-postgres.sh` during C01 host bootstrap:
...
`pg_stat_statements` is enabled in `postgresql.conf`:
...
All application roles use **scram-sha-256** authentication. Passwords are 32-byte random strings generated by `openssl rand -base64 32` (C01 `generate-secrets.sh`).
...
|-----------|-------|-----------|
| `shared_buffers` | `512 MB` | 6.25% of 8 GB RAM. Conservative for mixed workload (ingest + queries). PostgreSQL recommends 15–25% for dedicated DB servers, but Tian'er shares the CM5 with other containers. |
...
Nightly `pg_basebackup` via the `tianer` user's crontab (or systemd timer, to be defined in C12):
